...
if [ "$CIRCLE_BRANCH" == "dev" ] || [ "$CIRCLE_BRANCH" == "master" ]; then
  printf "${GREEN_COLOUR}Building base branch $PR_OR_DESTINATION_BRANCH_NAME which is at version $MVN_VERSION.${NO_COLOUR}\n"
else
  #Get the PR title
  export PR_TITLE=$(curl -s https://api.github.com/repos/$REPO/pulls/${CIRCLE_PULL_REQUEST##*/} | grep -Po '(?<="title":[[:space:]]")[^"]*(?=",)')

  printf "${GREEN_COLOUR}Building PR #${CIRCLE_PULL_REQUEST##*/} '$PR_TITLE' from branch $PR_OR_DESTINATION_BRANCH_NAME (into $DEV_BRANCH)${NO_COLOUR}\n"
  ...
  if [ "$CIRCLE_BRANCH" == "$RELEASE_BRANCH" ] && [ "$PR_OR_DESTINATION_BRANCH_NAME" != "$DEV_BRANCH" ]; then
